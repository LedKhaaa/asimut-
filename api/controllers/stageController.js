...
const genererConvention = require('../utils/generateConvention');
...
const createStage = async (req, res) => {
    try {
        ...
        if (!id_eleve || !entreprise) return res.status(400).json({ message: 'id_eleve et entreprise sont obligatoires' });
        if (!Number.isInteger(id_eleve)) return res.status(400).json({ message: 'id_eleve doit être un entier' });
        if (typeof entreprise !== 'string' || entreprise.length > 150) return res.status(400).json({ message: 'Entreprise invalide (max 150 caractères)' });
        if (contact_email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(contact_email)) return res.status(400).json({ message: 'Email contact invalide' });
        if (nb_lettres_envoyees !== undefined && !Number.isInteger(nb_lettres_envoyees)) return res.status(400).json({ message: 'nb_lettres_envoyees doit être un entier' });
        if (nb_lettres_recues !== undefined && !Number.isInteger(nb_lettres_recues)) return res.status(400).json({ message: 'nb_lettres_recues doit être un entier' });
...
const updateStage = async (req, res) => {
    try {
        ...
        if (!entreprise) return res.status(400).json({ message: 'entreprise est obligatoire' });
        if (typeof entreprise !== 'string' || entreprise.length > 150) return res.status(400).json({ message: 'Entreprise invalide (max 150 caractères)' });
        if (contact_email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(contact_email)) return res.status(400).json({ message: 'Email contact invalide' });
        if (nb_lettres_envoyees !== undefined && !Number.isInteger(nb_lettres_envoyees)) return res.status(400).json({ message: 'nb_lettres_envoyees doit être un entier' });
        if (nb_lettres_recues !== undefined && !Number.isInteger(nb_lettres_recues)) return res.status(400).json({ message: 'nb_lettres_recues doit être un entier' });
...
const exportPDF = async (req, res) => {
    try {
        const stage = await Stage.getById(parseInt(req.params.id));
        if (!stage) return res.status(404).json({ error: 'Stage introuvable' });

        const eleve = await Eleve.getById(stage.id_eleve);
        if (!eleve) return res.status(404).json({ error: 'Élève introuvable' });

        genererConvention(stage, eleve, res);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
};

module.exports = { getAllStages, getStageById, getStagesByEleve, createStage, updateStage, deleteStage, exportPDF };